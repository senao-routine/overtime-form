// Google Apps Script用のコード（職員用）
// 職員用スプレッドシートにデプロイして、ウェブアプリとして公開する
// 教員用との違い: 承認者が「事務長」のみ
...
// 期間シート列構成（12列）:
// A:申請日時 B:申請種類 C:職員名 D:メールアドレス E:クラブ名
...
function getOrCreateSheet(ss, sheetName) {
  ...
  if (!sheet) {
    ...
    sheet.setColumnWidth(3, 150);  // 職員名
    ...
    sheet.setColumnWidth(12, 100); // 承認済み
    ...
    // 承認チェックボックス（K列 = 11列目、事務長のみ）
    const approvalRange = sheet.getRange(2, 11, 999, 1);
    approvalRange.insertCheckboxes();

    // 承認済み列の条件付き書式（L列 = 12列目）
    const approvalStatusRange = sheet.getRange(2, 12, 999, 1);
...
function doGet(e) {
  ...
  if (type === 'staff') return getStaffMaster();
  if (type === 'clubs') return getClubsMaster();

  return ContentService.createTextOutput(JSON.stringify({
    status: 'active',
    message: '職員 時間外勤務申請処理APIが正常に動作しています'
...
function getStaffMaster() {
  try {
    ...
    const sheet = ss.getSheetByName('職員マスタ');
    ...
    Logger.log('職員マスタ取得エラー: ' + error.toString());
...
function doPost(e) {
  try {
    ...
    sheet.getRange(2, 1, 1, 12).setBorder(true, true, true, true, true, true);
    ...
    sheet.getRange(2, 11, 1, 1).insertCheckboxes();

    // Looker Studio統合シートにも追加
    try {
      updateLookerStudioSummary(sheetName, {
        applicationType: data.applicationType || '',
        staffName: data.teacherName || '',
        staffEmail: data.teacherEmail || '',
...
function sendConfirmationEmail(info) {
  ...
  Logger.log('控えメール送信完了: ' + info.staffEmail);
}
...
// Looker Studio統合シート列構成（14列）:
// A:職員名 B:メールアドレス C:申請種類 D:期間 E:クラブ名
...
function updateLookerStudioSummary(period, data) {
  ...
  if (!lookerSheet) {
    ...
    lookerSheet.setColumnWidth(1, 150);  // 職員名
    ...
    lookerSheet.setColumnWidth(13, 100); // 承認済み
    lookerSheet.setColumnWidth(14, 150); // 最終更新
  ...
  const newRow = [
    data.staffName, data.staffEmail,
...
/**
 * 期間シートの承認状態を更新する関数
 * 期間シート: K=事務長(11), L=承認済み(12)
 */
function updateApprovalStatus(sheet, row) {
  const approved = sheet.getRange(row, 11).getValue() === true;
 ...
 * 期間シート: D=メールアドレス(4), F=活動日(6), G=開始時間(7), K=事務長(11)
 * 統合シート: B=メールアドレス(2), F=活動日(6), G=開始時間(7), L=事務長(12), M=承認済み(13)
 */
function syncApprovalToLookerStudio(periodSheet, row) {
  try {
    ...
    const periodRow = periodSheet.getRange(row, 1, 1, 12).getValues()[0];
    const staffEmail = periodRow[3].toString().trim();  // D列: メールアドレス
    const activityDate = periodRow[5];                   // F列: 活動日
    const startTime = periodRow[6];                      // G列: 開始時間

    const businessApproved = periodRow[10];              // K列: 事務長
    ...
    for (let i = 0; i < lookerData.length; i++) {
      ...
      if (lEmail === staffEmail && lDate === activityDateStr && lStart === startTimeStr) {
        const targetRow = i + 2;
        lookerSheet.getRange(targetRow, 12).setValue(businessApproved);
        lookerSheet.getRange(targetRow, 13).setValue(businessApproved === true ? '承認済み' : '');
        break;
      }
    }
  } catch (error) {
